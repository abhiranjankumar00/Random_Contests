...
long long  waysUp[sz], waysDown[sz];
...
int main()
{
	...
	for(int i = 2; i <= sz-1; ++i) {
		waysUp[i] = waysUp[i-1] + waysUp[i-2];
		waysUp[i] %= Mod;
	}
	...
	while(test--) {
		int d;
		cin >> d;
		printf("%d: Up = %lld, Down = %lld\n", d, waysUp[d], waysDown[d]);
		assert(d < sz);
		long long ans = waysUp[d] * waysDown[d];
		ans %= Mod;
		ans = (ans + Mod) % Mod;
		cout << ans << "\n";
	}
