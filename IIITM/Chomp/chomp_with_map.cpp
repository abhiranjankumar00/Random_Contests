...
//		assert(0 <= row2 && row2 <= row1);
//		assert(0 <= row3 && row3 <= row2);
//		assert(0 <= row3 && row3 <= row2 && row2 <= row1 && row1 <= 100);
	}
};
...
int PN (int row1, int row2, int row3){
	map <Node, int> :: iterator it = m.find(Node(row1, row2, row3));
...
//	Pf("%d %d %d\n", row1, row2, row3);
	...
	rep(r3, row3-1, 0)
		if(PN(row1, row2, r3) == P)
			return m[Node(row1, row2, row3)] = N;
	...
	return m[Node(row1, row2, row3)] = P;
}
