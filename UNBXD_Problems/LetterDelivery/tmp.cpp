...
int solve(int side, int l, int r, int t) {
	...
	if(side == 0 && l == 0 && r == 2)
		DEBUG("#1");
	if(t > mxTime[id])  {
	//	Pf("id = %d, pos = %d, mxTime = %d, l = %d, r = %d, t = %d, ret = %d\n", id, side == L ? l : r, mxTime[id], l, r, t, -1 );
		return -1;
	}
	...
	if(side == 0 && l == 0 && r == 2)
		DEBUG("#2");
	if(l == 0 && r == N-1)  {
	//	Pf("id = %d, pos = %d, mxTime = %d, l = %d, r = %d, t = %d, ret = %d\n", id, side == L ? l : r, mxTime[id], l, r, t, t );
		return t;
	}
	...
	//Pf("id = %d, pos = %d, mxTime = %d, l = %d, r = %d, t = %d, ret = %d\n", id, side == L ? l : r, mxTime[id], l, r, t,  ans);
	return ret[nd]= ans;
}
...
int main()
{
	...
	tr(it, ret)
		Pf("%d %d %d %d = %d\n", it->first.side == L ? it->first.l : it->first.r, it->first.l, it->first.r, it->first.t, it->second);
	cout << ans << endl;
	return 0;
}
